...
// 1. Fix Imports
const zapLineIndex = lines.findIndex(l => l.includes('Zap'));
if (zapLineIndex !== -1 && !lines[zapLineIndex].includes('HelpCircle')) {
    lines[zapLineIndex] = lines[zapLineIndex].replace('Zap', 'Zap, HelpCircle');
}

// 2. Fix handleResetData breakage
const resetBrokenIndex = lines.findIndex(l => l.trim() === '};' && lines[lines.indexOf(l)+1]?.trim().startsWith('if (!user || resetLoading)'));
if (resetBrokenIndex !== -1) {
    lines[resetBrokenIndex + 1] = '    const handleResetData = async () => {\n' + lines[resetBrokenIndex + 1];
}

// 3. Add Modal (if not present)
if (!lines.some(l => l.includes('Modal de Detalhes da Partida'))) {
    const styleIndex = lines.findIndex(l => l.includes('<style>{`'));
    if (styleIndex !== -1) {
        const modalCode = `
        ...
        lines.splice(styleIndex, 0, modalCode);
    }
}

fs.writeFileSync(filePath, lines.join('\n'));
console.log('Fixed InputData.tsx structural errors.');
